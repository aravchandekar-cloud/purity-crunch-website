...
**Goal:** Add a persistent scroll-choreographed WebGL scene of procedural 3D banana chips behind all site content, with automatic 2D fallback.

**Architecture:** One fixed full-viewport Three.js canvas at `z-index: 1`; section content wrappers raised to `z-index: 2` so chips float over section backgrounds but under text/cards. GSAP ScrollTrigger (already Lenis-proxied via `gsap.ticker` in `js/smooth-scroll.js`) tweens a shared `state` object per section; the render loop applies state to instanced chip transforms.

**Tech Stack:** Three.js 0.160.0 (ESM via CDN importmap — no build step), GSAP 3.12.5 + ScrollTrigger (already loaded as globals), Lenis (already wired).

**Spec:** `docs/superpowers/specs/2026-06-12-3d-webgl-redesign-design.md`

**No test framework exists in this plain-HTML project; verification is browser-based (HTTP server + console + screenshots), matching the spec's Testing section.**

---

### Task 1: Create `js/three-scene.js`
...
Run: `node --input-type=module --check < js/three-scene.js` (or `node --check js/three-scene.js` on Node 20+)
...
git commit -m "feat: add Three.js procedural chip scene with scroll choreography"
```

---

### Task 2: Wire into `index.html`

**Files:**
- Modify: `index.html:19` (after ScrollTrigger script — add importmap)
- Modify: `index.html:22` (after `<body>` open — add canvas)
- Modify: `index.html:800` (after gsap-animations.js — add module script)

- [ ] **Step 1: Add importmap in `<head>` directly after the ScrollTrigger `<script>` (line 19)**
...
- [ ] **Step 2: Add the WebGL canvas as first child of `<body>` (before the cursor divs at line 23)**

```html
    <canvas id="webgl-bg" aria-hidden="true"></canvas>
```

- [ ] **Step 3: Add module script after the `gsap-animations.js` tag (line 800)**
...
```bash
git add index.html
git commit -m "feat: wire Three.js importmap, webgl canvas, and scene module into page"
```

---

### Task 3: CSS stacking layers

**Files:**
- Modify: `css/styles.css` (append layer rules at end of file)

- [ ] **Step 1: Append 3D layer rules at end of `css/styles.css`**

```css
/* ── 3D WebGL background layer ─────────────────────────────────────── */
/* Canvas sits above section backgrounds (z-1), content wrappers above it (z-2).
   #hero-canvas keeps z-index:0 via its existing ID rule (higher specificity). */
#webgl-bg {
    position: fixed;
    inset: 0;
    width: 100%;
...
git commit -m "feat: stacking layers so 3D chips render over backgrounds, under content"
```

---

### Task 4: Browser verification (spec Testing section)

**Files:** none (verification only)

- [ ] **Step 1: Serve the site**

Run: `python -m http.server 8787` (from project root, background)
Expected: `Serving HTTP on :: port 8787`

- [ ] **Step 2: Console check**

Open `http://localhost:8787/` in browser tooling. Read console.
Expected: `[3D] WebGL chip scene initialized: 52 chips`; zero errors (favicon.ico 404 is pre-existing, ignore).

- [ ] **Step 3: Visual check at 4 scroll positions**

Screenshot at: hero (top), `#products`, `#process`, `#export`.
Expected: chips visible floating over section backgrounds; headline/cards/forms fully readable above chips; ring formation at products; sparse faint chips at export.

- [ ] **Step 4: Interaction check**

Verify nav links, product quick-view modal open/close, FAQ accordion still work (canvas is `pointer-events: none`).

- [ ] **Step 5: Mobile width check**

Resize viewport to 390×844, reload.
Expected: console reports `24 chips`; no horizontal overflow.

- [ ] **Step 6: Reduced-motion check**

Emulate `prefers-reduced-motion: reduce`, reload.
Expected: no `[3D]` init log; 2D `#hero-canvas` visible in hero; page functions normally.
...
git status --short
```
Commit any verification fixes with descriptive messages.

---

## Self-Review (done at write time)

- **Spec coverage:** initWebGL/fallback (T1 init+try/catch), ChipFactory + 4 variants incl. speckle (T1), InstancedMesh counts 52/24 (T1), lighting+fog (T1), camera parallax (T1), choreography incl. hero/trust/products-ring/heritage/process/tail (T1 FORMATIONS), importmap+canvas+module (T2), z-layers (T3 — chips render *over* backgrounds rather than backgrounds made translucent; simpler, same visual goal, avoids re-theming dark sections), context-lost/visibility/reduced-motion (T1), testing steps (T4). ✓
- **Placeholder scan:** none. ✓
- **Type consistency:** `state` fields match between FORMATIONS/apply/tick; `chips[]` fields match tick usage. ✓
